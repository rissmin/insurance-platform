...
export function rankProducts(
  ...
  const scored: Recommendation[] = products.map((product) => {
    ...
    if (
      ...
      score += 4;
      reasons.push("중장년층 또는 간편 가입 관점에서 검토하기 좋은 상품입니다.");
    }
...
function buildCustomerAnalysis(
  ...
  return `${ageText} ${budgetText}, 고객님은 ${values.goal} 목적에 맞는 보장을 찾고 계시고 실제로 오래 유지 가능한 보험료도 중요하게 보고 계신 상황으로 해석됩니다. ${product.name}은(는) 그 기준에 비교적 잘 맞는 편입니다.`;
}
